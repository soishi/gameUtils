...
describe("swipe utilities", () => {
  ...
  describe("getSwipeDirection", () => {
    test("水平方向のスワイプを正しく判定する", () => {
      // 右方向
      expect(getSwipeDirection({ x: 100, y: 100 }, { x: 200, y: 100 })).toBe(
        "right",
      );

      // 左方向
      expect(getSwipeDirection({ x: 200, y: 100 }, { x: 100, y: 100 })).toBe(
        "left",
      );

      // 斜め右上（水平成分が大きい）
      expect(getSwipeDirection({ x: 100, y: 100 }, { x: 200, y: 120 })).toBe(
        "right",
      );

      // 斜め左下（水平成分が大きい）
      expect(getSwipeDirection({ x: 200, y: 100 }, { x: 100, y: 120 })).toBe(
        "left",
      );
    });
    ...
    test("垂直方向の動きが大きい場合は null を返す", () => {
      // 真上
      expect(getSwipeDirection({ x: 100, y: 100 }, { x: 100, y: 50 })).toBe(
        null,
      );

      // 真下
      expect(getSwipeDirection({ x: 100, y: 100 }, { x: 100, y: 200 })).toBe(
        null,
      );

      // 垂直成分が水平成分より大きい
      expect(getSwipeDirection({ x: 100, y: 100 }, { x: 120, y: 200 })).toBe(
        null,
      );
    });
  });
